...
export const CustomerService={
 async dashboard(userId:string){const wallet=await CustomerRepository.getWallet(userId);const [pendingCount,completedCount,recentOrders,recentTransactions,announcementSetting]=await Promise.all([CustomerRepository.countOrders(userId,'PENDING'),CustomerRepository.countOrders(userId,'COMPLETED'),CustomerRepository.recentOrders(userId),CustomerRepository.recentTransactions(wallet.id),prisma.setting.findUnique({where:{key:'global_announcement'}})]);let announcement=null;if(announcementSetting){try{announcement=JSON.parse(announcementSetting.value)}catch(e){}}return {wallet:{availableBalance:Number(wallet.availableBalance),pendingBalance:Number(wallet.pendingBalance),lifetimeFunded:Number(wallet.lifetimeFunded),lifetimeSpent:Number(wallet.lifetimeSpent)},orderStats:{pendingCount,completedCount},recentOrders,recentTransactions,actionLinks:{fundWallet:'/wallet/fund',browseCatalog:'/catalog'},announcement}},
 async listOrders(userId:string,page:number,limit:number,status?:OrderStatus){const result=await CustomerRepository.listOrders(userId,page,limit,status);return {...result,items:result.items.map(order=>toCustomerOrderView(order,false))}},
 async getOrder(userId:string,id:string){const order=owned(await CustomerRepository.findOrderDetails(id),'Order',userId);const receipt=await CustomerRepository.findOrderReceipt(id);return {...toCustomerOrderView(order,true),receiptReference:receipt?.reference??null}},
