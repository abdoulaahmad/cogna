...
export default async function walletRoutes(app: FastifyInstance) {
  ...
  app.register(async (child) => {
    ...
    child.get('/transactions', async (req: FastifyRequest, reply: FastifyReply) => {
      try {
        const { sub } = req.user as { sub: string }
    ...
    child.get('/fundings', async (req: FastifyRequest, reply: FastifyReply) => {
      try {
        const { sub } = req.user as { sub: string }
    ...
    child.get('/crypto-rate', async (_req: FastifyRequest, reply: FastifyReply) => {
      try {
    ...
    child.get('/fundings/:reference/verify', async (req: FastifyRequest, reply: FastifyReply) => {
      try {
        const { sub } = req.user as { sub: string }
    ...
    child.post('/refunds', async (req: FastifyRequest, reply: FastifyReply) => {
      try {
        const { sub } = req.user as { sub: string }
    ...
    child.post('/purchase', async (req: FastifyRequest, reply: FastifyReply) => {
      try {
        const { sub } = req.user as { sub: string }
    ...
    child.post('/fund', async (req: FastifyRequest, reply: FastifyReply) => {
      try {
     ...
     */
    child.post('/fund/crypto', async (req: FastifyRequest, reply: FastifyReply) => {
      try {
        let { sub, email } = req.user as { sub: string; email?: string }
        if (!email) {
          const user = await (await import('@/repositories/user.repository')).UserRepository.findById(sub)
