...
function ResetPasswordContent() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.data.success) throw new Error(response.data.message || 'Unable to reset password.');
      setSuccess(true);
      setTimeout(() => {
  ...
  return (
    <section className="relative w-full max-w-md rounded-[2rem] border border-emerald-100/15 bg-white/[0.07] p-7 shadow-premium-dark backdrop-blur-xl sm:p-9">
      ...
      <h1 className="mt-5 font-display text-3xl font-bold">New Password</h1>
      ...
      {!success ? (
        <>
          ...
          <form onSubmit={submit} className="mt-7 space-y-4">
            <label className="block text-xs font-bold text-emerald-100/75">Reset Code
            ...
            <button disabled={loading || token.length < 6 || password.length < 8} className="mt-6 flex w-full items-center justify-center gap-2 rounded-full bg-[#D4AF37] px-5 py-3.5 text-sm font-bold text-[#062C23] transition hover:bg-[#F8D56B] disabled:opacity-50">
              {loading ? <Loader2 className="animate-spin" size={17} /> : <>Reset Password <ArrowRight size={17} /></>}
            </button>
          </form>
        </>
      ) : (
        <div className="mt-8 flex flex-col items-center justify-center space-y-4 py-4 text-center">
          ...
          <p className="text-lg font-bold text-white">Password reset successfully!</p>
